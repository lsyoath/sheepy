...
ROIs='lvCa rvCa lNAC rNAC ldCa rdCa'
...
rm -f Clust_mask+*
 3dclust -1Dformat -nosum -1dindex 1 -1tindex 1 -1thresh ${th1NC} -dxyz=1 -savemask Clust_mask 1.01 26.1 m_NC_${roi}+tlrc
  
  3dcalc -a Clust_mask+tlrc -b m_NC_${roi}+tlrc -expr 'step(a)*b' -prefix ${rtpath}/MCfc/mc_NC_${roi} 

 rm -f Clust_mask+*
 3dclust -1Dformat -nosum -1dindex 1 -1tindex 1 -1thresh ${th1aMCI} -dxyz=1 -savemask Clust_mask 1.01 26.1 m_aMCI_${roi}+tlrc
  
  3dcalc -a Clust_mask+tlrc -b m_aMCI_${roi}+tlrc -expr 'step(a)*b' -prefix ${rtpath}/MCfc/mc_aMCI_${roi} 

 rm -f Clust_mask+*
 3dclust -1Dformat -nosum -1dindex 1 -1tindex 1 -1thresh ${th1svMCI} -dxyz=1 -savemask Clust_mask 1.01 26.1 m_svMCI_${roi}+tlrc
